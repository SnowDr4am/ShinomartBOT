...
async def handle_feedback(callback: CallbackQuery, state: FSMContext):
    ...
    if choice == "complain":
        ...
        await callback.message.answer(
            "<b>У вас есть идея, как сделать нас лучше? 💡</b>\n\n"
            "Опишите её в следующем сообщении. Мы внимательно всё прочитаем и постараемся воплотить лучшие предложения!\n\n"
            "или напишите <code>отмена</code> для отмены",
            parse_mode='HTML'
        )

...
async def process_appointment_response(callback: CallbackQuery, bot: Bot):
    ...
    if action == "yes":
        if await rq.confirm_appointment(user_id):
            await callback.message.edit_text(
                ...
                "⏰ <u>Мы ждём вас в назначенное время!</u>",
                parse_mode='HTML',
                reply_markup=None
            )
        else:
            await callback.message.edit_text(
                ...
                "ℹ️ <u>Проверьте статус записи или обратитесь в поддержку.</u>",
                parse_mode='HTML',
                reply_markup=None
            )
        ...
        if await rq.delete_appointment(user_id):
            await callback.message.edit_text(
                "❌ <b>Запись отменена.</b> ✅\n\n"
                "ℹ️ <u>Вы можете записаться заново через меню.</u>",
                parse_mode='HTML',
                reply_markup=None
            )
        else:
            await callback.message.edit_text(
                "⚠️ <b>Ошибка:</b> Запись не найдена 😕\n\n"
                "ℹ️ <u>Возможно, запись уже была отменена</u>",
                parse_mode='HTML',
                reply_markup=None
            )
    ...
    if daily_message_id:
        try:
            ...
            await callback.message.edit_text(
                callback.message.text + "\n\n⚠️ Не удалось обновить отчёт в канале.",
                reply_markup=None
            )
...
async def update_channel_message(bot: Bot):
    ...
    if daily_message_id:
        ...
        print("Не найден daily_message_id для обновления канала")